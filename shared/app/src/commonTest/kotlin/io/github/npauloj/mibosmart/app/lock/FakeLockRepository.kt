 ...
 * @param answer runs inside every **read**, before it returns: a gate a test can hold open, or
 * a failure to raise.
 * @param answerWrite the same for every **write**.
 * @param remoteOpenAfterEnabling what `status-abrir-remoto` answers **after** a successful
 * `habilitar-abrir-remoto`.
 * @param answerConfirmation runs inside `status-abertura` **only after a command was sent** —
 * the confirmation read of SPEC L3.
 * @param obeysCommands whether `controle-fechadura` actually moves the door.
 * @param history what `historico-abertura` answers, in the partner's own order.
 */
internal class FakeLockRepository(
     ...
     * `controle-fechadura` acknowledges the command, and whether the hardware follows is a
     * separate question (SPEC L3).
     */
    ...
    /**
     * The grant only takes effect if the call itself succeeded — [answerWrite] runs first, and
     * a failing one leaves the flag exactly where it was, like the partner would.
     */
