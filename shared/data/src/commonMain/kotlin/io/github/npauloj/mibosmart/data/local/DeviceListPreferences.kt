 ...
 *
 * The same database as the device cache and not a second store: one file to open, one migration
 * path, one thing to clear. The rows are keyed by name so the next preference is a constant here
 * rather than a column, a migration and a driver change.
 */
internal class SqlDeviceListPreferences(driverFactory: DatabaseDriverFactory) : DeviceListPreferences {
    ...
    /**
     * A value written by an older build — or by a newer one, after a downgrade — is not a choice
     * this build can honour, so it reads as the default rather than as a failure: SPEC D4's fallback
     * is `todos`, and a list that refuses to open because of a remembered chip would be worse than
     * one that opens on the wrong chip.
     */
    ...
    /**
     * Stored by [Enum.name], not by ordinal: reordering the enum would silently reinterpret every
     * stored ordinal, and the one value here is worth less than that class of bug.
     */
